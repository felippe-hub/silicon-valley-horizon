...
const HeroSection = () => {
  return (
    <section className="relative min-h-[100dvh] flex items-center overflow-hidden section-dark section-border-dark">
      ...
      <div className="relative z-10 mx-auto grid min-h-[100dvh] w-full max-w-[98rem] items-center gap-8 px-4 pb-12 pt-24 md:grid-cols-[0.5fr_1.5fr] md:px-8">
        {/* Left: Copy */}
        <div className="space-y-7">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-semibold tracking-[0.15em] uppercase glass-dark" style={{ color: "rgba(255,255,255,0.5)" }}>
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}
            className="font-display text-4xl md:text-5xl lg:text-[3.5rem] font-bold leading-[1] tracking-tight text-lets-white">
            Marketing médico<br />sem complicação<span style={{ color: "#36A9E1" }}>.</span>
          </motion.h1>

          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
            className="text-base md:text-lg max-w-md leading-relaxed" style={{ color: "rgba(255,255,255,0.55)" }}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="flex flex-wrap gap-4 pt-2">
            ...
            <a href="#processo" className="inline-flex items-center gap-2 glass-dark font-medium px-7 py-3.5 rounded-xl transition-all text-lets-white hover:bg-[rgba(255,255,255,0.08)]">
              Conheça o processo
            </a>
          </motion.div>
        </div>
        ...
        <div className="relative hidden md:flex items-center justify-center">
          <div className="relative w-full max-w-[620px] h-[520px]">
            ...
            <motion.div
              ...
              className="absolute top-0 right-0 w-[400px] h-[260px] image-frame-dark animate-float"
            ...
            >
              ...
              <div className="absolute inset-0 glass-dark-md flex items-end p-5">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-2 h-2 rounded-full animate-pulse-glow" style={{ background: "#36A9E1" }} />
                    <span className="text-[11px] font-medium tracking-[0.15em] uppercase" style={{ color: "rgba(255,255,255,0.5)" }}>Painel em tempo real</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    {[{ label: "Alcance", value: "+142%", Icon: Globe }, { label: "Engajamento", value: "+87%", Icon: Users }, { label: "Conversões", value: "+34%", Icon: TrendingUp }].map((s, i) => (
                      <div key={i} className="glass-dark rounded-lg p-2 text-center inner-shadow-dark">
            ...
            {/* LAYER 2: Doctor image with glass frame — 3D depth via perspective */}
            <motion.div
              initial={{ opacity: 0, x: -40, rotateY: 8 }}
              animate={{ opacity: 1, x: 0, rotateY: 0 }}
              transition={{ duration: 0.9, delay: 0.5 }}
              className="absolute bottom-8 left-0 w-[200px] animate-float-delayed z-10"
            ...
            >
              <div className="glass-dark-strong rounded-2xl p-3 glow-border inner-shadow-dark">
                ...
                <div className="text-center">
                  ...
                  <div className="text-[10px]" style={{ color: "rgba(255,255,255,0.4)" }}>Dermatologia</div>
                </div>
            ...
            {/* LAYER 3: Terminal with operation commands — most forward */}
            <motion.div
              initial={{ opacity: 0, y: 40, rotateX: -5 }}
              animate={{ opacity: 1, y: 0, rotateX: 0 }}
              transition={{ duration: 0.8, delay: 0.7 }}
              className="absolute bottom-12 right-8 w-[260px] glass-dark-strong rounded-2xl p-4 animate-float-slow glow-border inner-shadow-dark z-20"
            ...
            >
              <div className="flex items-center gap-1.5 mb-3">
                ...
                <span className="text-[10px] ml-2 font-mono" style={{ color: "rgba(255,255,255,0.3)" }}>letsdoc.terminal</span>
              </div>
              <div className="font-mono text-[11px] space-y-1.5">
                <div style={{ color: "rgba(255,255,255,0.4)" }}><span style={{ color: "#36A9E1" }}>$</span> iniciar operação</div>
                <div style={{ color: "rgba(255,255,255,0.5)" }}><span style={{ color: "#50C878" }}>✓</span> Estratégia definida</div>
                <div style={{ color: "rgba(255,255,255,0.5)" }}><span style={{ color: "#50C878" }}>✓</span> Conteúdo produzido</div>
                <div style={{ color: "rgba(255,255,255,0.5)" }}><span style={{ color: "#50C878" }}>✓</span> Tráfego ativado</div>
            ...
            {/* LAYER 4: Floating metric card — enters from right */}
            <motion.div
              initial={{ opacity: 0, x: 60 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.8, delay: 0.9 }}
              className="absolute top-[45%] right-0 glass-dark-md rounded-xl p-3 w-[150px] animate-float z-10 inner-shadow-dark"
            >
              <div className="flex items-center gap-2 mb-2">
                <BarChart3 size={14} style={{ color: "#36A9E1" }} />
                <span className="text-[10px] font-semibold text-lets-white">Performance</span>
              </div>
              <div className="flex items-end gap-[2px] h-8">
                {[30, 45, 55, 40, 65, 50, 72, 60, 80, 70, 90].map((h, i) => (
                  <motion.div key={i} initial={{ height: 0 }} animate={{ height: `${h}%` }}
                    transition={{ duration: 0.5, delay: 1 + i * 0.04 }}
                    className="flex-1 rounded-[2px]" style={{ background: `rgba(54,169,225,${0.2 + (h / 100) * 0.5})`, minHeight: 2 }} />
                ))}
              </div>
            </motion.div>
